...
const Projects = () => {
  return (
    <>
      <div className="flex flex-col w-full  h-full justify-center items-center gap-y-4 overflow-hidden " id="project">
        <div className="flex flex-col max-[1100px]:flex-wrap max-[700px]:flex-col w-full overflow-hidden">
          <p className="education text-3xl flex justify-center items-center">projects</p>
        ...
        <div className="w-full flex max-[1100px]:flex-wrap max-[1100px]:gap-y-12 justify-center items-center gap-x-8">
          {projectsData.map((project, index) => (
            <div key={index} className="sm:w-[25rem] sm:h-[30rem] w-[20rem] h-[25rem]  flex flex-col rounded-lg bg-[#182c35d9] shadow-lg shadow-slate-900">
              <div className="projectImage w-full h-full relative">
                <img className="w-full h-full rounded-t-lg object-cover" src={project.imageSrc} alt={project.name} />
              ...
              <div className="p-6 w-full h-full flex flex-col justify-center items-center gap-y-1">
                <h5 className="mb-2 text-xl font-medium leading-tight text-white">{project.name}</h5>
                <div className="w-24 h-8 bg-blue-600 rounded-md text-xl text-white text-center font-medium">
                  <a href={project.liveLink} target="_blank" rel="noopener noreferrer">see live</a>
                </div>
                <a className="w-full h-10 flex justify-center items-center gap-y-4" href={project.githubLink} target="_blank" rel="noopener noreferrer">
                  <div className="w-10 h-10 text-white">
                    <AiFillGithub className="w-10 h-10" />
                  </div>
                  <div className="text-xl text-white font-medium">project github link</div>
                </a>
                <div className="flex flex-col justify-center items-center gap-y-1">
                  <div className="flex justify-center items-center flex-wrap gap-x-3 gap-y-3">
                    {project.technologies.map((tech, techIndex) => (
                      <button key={techIndex} className="w-24 h-8 bg-white rounded-sm">{tech}</button>
                    ))}
                  </div>
                </div>
              </div>
